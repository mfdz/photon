    ...
    public Point apply(Request webRequest) throws BadRequestException {
        ...
        try {
            ...
            if (lon > 180.0 || lon < -180.00) {
                throw new BadRequestException(400, "invalid term 'lon', expected number >= -180.0 and <= 180.0");
            }
            Double lat = Double.valueOf(latParam);
            if (lat > 90.0 || lat < -90.00) {
                throw new BadRequestException(400, "invalid term 'lat', expected number >= -90.0 and <= 90.0");
            }
            location = geometryFactory.createPoint(new Coordinate(lon, lat));
        } catch (NullPointerException | NumberFormatException e) {
            throw new BadRequestException(400, "invalid term 'lat' and/or 'lon', try instead lat=51.5&lon=8.0");
        }
        return location;
    }
}
